...
RUN cd poppler-${version} && mkdir build && cd build && cmake .. -DCMAKE_INSTALL_PREFIX=/ -DCMAKE_BUILD_TYPE=release -DBUILD_GTK_TESTS=OFF -DBUILD_QT5_TESTS=OFF -DBUILD_CPP_TESTS=OFF -DENABLE_QT5=OFF -DENABLE_LIBOPENJPEG=openjpeg2 && make -j2
